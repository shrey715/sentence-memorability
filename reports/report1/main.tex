...
Paired Wilcoxon signed-rank tests (collapsed across noun conditions) showed no significant voice effect on Corrected IR ($V = 1916.5$, $p = .485$; Active $M = 0.668$, Passive $M = 0.677$) or WR Accuracy ($V = 3520.5$, $p = .114$; Active $M = 0.756$, Passive $M = 0.734$). Notably, the WR direction (active slightly higher) is consistent with the active-sentence recall bias documented by \citet{james1973}, though it does not reach significance here. Figure~\ref{fig:voice_main} shows per-participant connected paired dot plots, making the negligible within-participant shift apparent.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{plots/10_voice_paired_dots.png}
  \caption{Per-participant voice comparison (connected paired dot plot). Each line is one participant; lines are nearly horizontal for both IR~CR (left) and WR~Acc (right), confirming negligible voice main effects. Active $>$ Passive for WR is directionally consistent with active-sentence bias.}
...
% ============================================================
\section{\normalsize Discussion and Conclusion}
% ============================================================
...
% ============================================================
\section{\normalsize Code and Data Availability}
